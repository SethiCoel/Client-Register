...
def registro_anterior():
    ...
    while True:
        ...
        for tabela in tabelas:
            print(f'Registros do dia: {tabela[0]}')

        data_escolhida = str(input('\nDigite a data do registro que deseja ver: '))
        ...
        try:
            primeira_tabela = cursor.execute(f'SELECT * FROM "{data_escolhida}"')
            segunda_tabela = cursor.execute(f'SELECT * FROM "valor_{data_escolhida}"')
            break
        

        except Exception as error:
            Registro.sub_titulo(f'Registros Anteriores')
            Registro.cor(f'A tabela {data_escolhida} não foi encontrada | Error {error}', 'vermelho')
